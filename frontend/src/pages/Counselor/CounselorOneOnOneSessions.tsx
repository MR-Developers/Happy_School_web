...
function CounselorOneOnOneSessions() {
  ...
  useEffect(() => {
    ...
    const fetchFilteredTickets = async () => {
      ...
      try {
        ...
        // Flatten tickets from all schools and add school information
        const flattenedTickets: Ticket[] = [];
        response.data.data.forEach((schoolData) => {
          schoolData.tickets.forEach((ticket) => {
        ...
        console.log("Fetched tickets:", flattenedTickets);

        // Sort by timestamp
        const sorted = flattenedTickets.sort((a: Ticket, b: Ticket) => {
          const timeA = a.timestamp?._seconds
            ? a.timestamp._seconds * 1000
            : new Date(a.timestamp ?? "").getTime();
          const timeB = b.timestamp?._seconds
            ? b.timestamp._seconds * 1000
        ...
        if (
          response.data.failedSchools &&
          response.data.failedSchools.length > 0
        ) {
          setFailedSchools(response.data.failedSchools.map((f) => f.school));
        }
      } catch (err) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (oneononesessions: number) => (
        <Tag color="orange">
      ...
      {failedSchools.length > 0 && (
        <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            <strong>Note:</strong> Unable to fetch tickets from:{" "}
            {failedSchools.join(", ")}
          </p>
        </div>
      )}
...
export default CounselorOneOnOneSessions;
